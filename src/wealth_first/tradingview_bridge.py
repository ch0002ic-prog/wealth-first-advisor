...
class BridgeSettings:
    ...
    main2_returns_csv_path: Path | None = None
    ...
    main4_gate: str | None = None
    main4_gate_scale: str | None = None
    main4_min_robust_min_relative: float | None = None
    main4_n_folds: int | None = None
    main4_seed: int | None = None
    main4_promotion_date: str | None = None
    main4_promotion_status: str | None = None
    main4_promotion_label: str | None = None
    main4_promotion_friction_range: str | None = None
...
def load_bridge_settings_from_env(env: Mapping[str, str] | None = None) -> BridgeSettings:
    ...
    return BridgeSettings(
        ...
        main4_gate=_env_string(runtime_env, "WEALTH_FIRST_MAIN4_GATE"),
        main4_gate_scale=_env_string(runtime_env, "WEALTH_FIRST_MAIN4_GATE_SCALE"),
        main4_min_robust_min_relative=_env_float(runtime_env, "WEALTH_FIRST_MAIN4_MIN_ROBUST_MIN_RELATIVE"),
        main4_n_folds=_env_int(runtime_env, "WEALTH_FIRST_MAIN4_N_FOLDS"),
        main4_seed=_env_int(runtime_env, "WEALTH_FIRST_MAIN4_SEED"),
        main4_promotion_date=_env_string(runtime_env, "WEALTH_FIRST_MAIN4_PROMOTION_DATE"),
        main4_promotion_status=_env_string(runtime_env, "WEALTH_FIRST_MAIN4_PROMOTION_STATUS"),
        main4_promotion_label=_env_string(runtime_env, "WEALTH_FIRST_MAIN4_PROMOTION_LABEL"),
        main4_promotion_friction_range=_env_string(runtime_env, "WEALTH_FIRST_MAIN4_PROMOTION_FRICTION_RANGE"),
    )
...
def _resolve_main2_returns_csv_path(settings: BridgeSettings) -> Path:
    return settings.main2_returns_csv_path or PROJECT_ROOT / "data" / "demo_sleeves.csv"


def _resolve_main2_compare_detail_csv_path(settings: BridgeSettings) -> Path:
    if settings.main2_compare_detail_csv_path is not None:
        return settings.main2_compare_detail_csv_path

    artifact_root = _resolve_artifact_root_path(settings)
    preferred_path = artifact_root / "main2_2007_v8_best_detail.csv"
    if preferred_path.exists():
        return preferred_path

    best_candidates = sorted(artifact_root.glob("main2_*best*_detail.csv"))
    if best_candidates:
        return best_candidates[-1]

    detail_candidates = sorted(artifact_root.glob("main2_*_detail.csv"))
    if detail_candidates:
        return detail_candidates[-1]

    return preferred_path


def _sanitize_launch_label(value: str | None) -> str:
    cleaned = re.sub(r"[^a-z0-9]+", "_", (value or "").strip().lower()).strip("_")
    return cleaned or "live_run"


def _build_main2_launch_presets() -> list[dict[str, Any]]:
    return [
        {
            "id": "quick-probe",
            "label": "Quick Probe",
            "description": "Fast single-seed sanity run for the live board and artifact pipeline.",
            "estimated_runtime": "under a minute",
            "default_artifact_label": "live_quick_probe",
            "recommended": True,
            "args": [
                "--splits",
                "regime",
        ...
        },
        {
            "id": "board-refresh",
            "label": "Board Refresh",
            "description": "Light chrono plus regime refresh so the leaderboard picks up a new comparable main2 run.",
            "estimated_runtime": "a few minutes",
            "default_artifact_label": "live_board_refresh",
            "recommended": False,
            "args": [
                "--splits",
                "regime",
...
def _build_main2_progress_summary(progress_log_path: str | Path | None) -> dict[str, Any]:
    event_count, recent_events = _tail_jsonl_records(progress_log_path, limit=24)
    latest_event = recent_events[0] if recent_events else None
    latest_checkpoint = next((event for event in recent_events if event.get("event_type") == "training_checkpoint"), None)
    latest_fold_result = next((event for event in recent_events if event.get("event_type") == "fold_completed"), None)
    checkpoint_history = [event for event in recent_events if event.get("event_type") == "training_checkpoint"]
    context_event = next(
        (event for event in recent_events if event.get("split") is not None and event.get("fold") is not None),
        latest_checkpoint,
    )

    total_folds = _coerce_progress_int((latest_event or {}).get("total_folds"))
    if total_folds is None:
        total_folds = _coerce_progress_int((latest_checkpoint or {}).get("total_folds"))

    completed_folds = _coerce_progress_int((latest_event or {}).get("completed_folds"))
    if completed_folds is None:
        completed_folds = _coerce_progress_int((latest_fold_result or {}).get("completed_folds"))
    if completed_folds is None:
        completed_folds = 0

    latest_event_type = (latest_event or {}).get("event_type")
    step_progress_fraction = _coerce_progress_float((latest_checkpoint or {}).get("progress_fraction"))
    active_step = _coerce_progress_int((latest_checkpoint or {}).get("step"))
    train_steps = _coerce_progress_int((latest_checkpoint or {}).get("train_steps"))
    latest_validation_score = _coerce_progress_float((latest_checkpoint or {}).get("validation_score"))
    best_validation_score = _coerce_progress_float((latest_checkpoint or {}).get("best_validation_score"))
    latest_loss = _coerce_progress_float((latest_checkpoint or {}).get("latest_loss"))
    latest_epsilon = _coerce_progress_float((latest_checkpoint or {}).get("epsilon"))
    latest_replay_size = _coerce_progress_int((latest_checkpoint or {}).get("replay_size"))
    latest_episode_reset_count = _coerce_progress_int((latest_checkpoint or {}).get("episode_reset_count"))
    validation_slice_scores = [
        score
        for score in (
            _coerce_progress_float(value) for value in ((latest_checkpoint or {}).get("validation_slice_scores") or [])
        )
        if score is not None
    ]

    overall_progress_fraction: float | None = None
    if latest_event_type == "run_completed":
        overall_progress_fraction = 1.0
    elif total_folds and total_folds > 0:
        fold_progress = float(completed_folds)
        if latest_event_type == "training_checkpoint" and step_progress_fraction is not None:
            fold_progress += step_progress_fraction
        overall_progress_fraction = min(max(fold_progress / total_folds, 0.0), 1.0)

    if latest_event_type == "run_started":
        stage_label = "Preparing adaptive DRL run"
    elif latest_event_type == "fold_started":
        stage_label = "Training fold"
    elif latest_event_type == "training_checkpoint":
        stage_label = "Training checkpoint"
    elif latest_event_type == "fold_completed":
        stage_label = "Fold completed"
    elif latest_event_type == "run_completed":
        stage_label = "Adaptive DRL run completed"
    elif latest_event_type == "run_failed":
        stage_label = "Adaptive DRL run failed"
    else:
        stage_label = "Waiting for telemetry"

    latest_fold_metrics = None
    if latest_fold_result is not None:
        latest_fold_metrics = {
            "split": latest_fold_result.get("split"),
            "seed": latest_fold_result.get("seed"),
            "fold": latest_fold_result.get("fold"),
            "runtime_seconds": _coerce_progress_float(latest_fold_result.get("runtime_seconds")),
            "validation_total_return": _coerce_progress_float(latest_fold_result.get("validation_total_return")),
            "test_total_return": _coerce_progress_float(latest_fold_result.get("test_total_return")),
            "validation_average_spy_weight": _coerce_progress_float(latest_fold_result.get("validation_average_spy_weight")),
            "test_average_spy_weight": _coerce_progress_float(latest_fold_result.get("test_average_spy_weight")),
            "validation_average_turnover": _coerce_progress_float(latest_fold_result.get("validation_average_turnover")),
            "test_average_turnover": _coerce_progress_float(latest_fold_result.get("test_average_turnover")),
        }
    ...
    checkpoint_history_rows = [
        {
            ...
            "step": _coerce_progress_int(checkpoint.get("step")),
            "train_steps": _coerce_progress_int(checkpoint.get("train_steps")),
            "progress_fraction": _coerce_progress_float(checkpoint.get("progress_fraction")),
            "validation_score": _coerce_progress_float(checkpoint.get("validation_score")),
            "best_validation_score": _coerce_progress_float(checkpoint.get("best_validation_score")),
            "latest_loss": _coerce_progress_float(checkpoint.get("latest_loss")),
            "epsilon": _coerce_progress_float(checkpoint.get("epsilon")),
            "replay_size": _coerce_progress_int(checkpoint.get("replay_size")),
            "episode_reset_count": _coerce_progress_int(checkpoint.get("episode_reset_count")),
            "checkpoint_improved": checkpoint.get("checkpoint_improved"),
        }
        for checkpoint in reversed(checkpoint_history)
    ]

    return {
        "event_count": event_count,
        "latest_event_type": latest_event_type,
        "latest_event_timestamp": (latest_event or {}).get("timestamp"),
        "stage_label": stage_label,
        "total_folds": total_folds,
        "completed_folds": completed_folds,
        "active_split": (context_event or {}).get("split"),
        "active_seed": (context_event or {}).get("seed"),
        "active_fold": (context_event or {}).get("fold"),
        "active_step": active_step,
        "train_steps": train_steps,
        "step_progress_fraction": step_progress_fraction,
        ...
        "latest_replay_size": latest_replay_size,
        "latest_episode_reset_count": latest_episode_reset_count,
        "validation_slice_scores": validation_slice_scores,
        "checkpoint_improved": (latest_checkpoint or {}).get("checkpoint_improved"),
        "checkpoint_history": checkpoint_history_rows,
        "latest_fold_metrics": latest_fold_metrics,
        "recent_events": recent_events,
    }


def _serialize_main2_launch_job(job: dict[str, Any] | None) -> dict[str, Any] | None:
    if job is None:
        return None

    serialized = dict(job)
    serialized["detail_exists"] = Path(str(job["detail_path"])).exists()
    serialized["summary_exists"] = Path(str(job["summary_path"])).exists()
    serialized["comparison_summary_exists"] = Path(str(job["comparison_summary_path"])).exists()
    serialized["log_tail"] = _read_log_tail(job.get("log_path"))
    serialized["progress_log_exists"] = Path(str(job["progress_log_path"])).exists() if job.get("progress_log_path") else False
    serialized["progress"] = _build_main2_progress_summary(job.get("progress_log_path"))
    return _json_ready(serialized)


def _refresh_main2_launch_state(app: FastAPI) -> None:
    with app.state.main2_launch_lock:
        active_job = app.state.main2_active_launch
        process = app.state.main2_launch_process
        if active_job is None or process is None:
            return
        ...
        completed_job["status"] = "completed" if exit_code == 0 else "failed"
        completed_job["exit_code"] = int(exit_code)
        completed_job["completed_at"] = datetime.now(UTC).isoformat()

        log_handle = app.state.main2_launch_log_handle
        if log_handle is not None and not log_handle.closed:
            log_handle.close()

        app.state.main2_launch_log_handle = None
        app.state.main2_launch_process = None
        app.state.main2_active_launch = None
        app.state.main2_latest_launch = completed_job


def _cleanup_main2_launch_process(app: FastAPI) -> None:
    _refresh_main2_launch_state(app)
    with app.state.main2_launch_lock:
        process = app.state.main2_launch_process
        active_job = app.state.main2_active_launch
        log_handle = app.state.main2_launch_log_handle
        ...
        if process is not None and process.poll() is None:
            ...
            if active_job is not None:
                terminated_job = dict(active_job)
                terminated_job["status"] = "terminated"
                terminated_job["exit_code"] = int(process.returncode) if process.returncode is not None else -15
                terminated_job["completed_at"] = datetime.now(UTC).isoformat()
                app.state.main2_latest_launch = terminated_job
...
def _start_main2_launch(settings: BridgeSettings, app: FastAPI, preset_id: str, artifact_label: str | None) -> dict[str, Any]:
    _refresh_main2_launch_state(app)

    preset = next((candidate for candidate in _build_main2_launch_presets() if candidate["id"] == preset_id), None)
    if preset is None:
        raise HTTPException(status_code=400, detail=f"Unknown main2 launch preset '{preset_id}'.")

    returns_csv_path = _resolve_main2_returns_csv_path(settings)
    if not returns_csv_path.exists():
        raise HTTPException(status_code=400, detail=f"main2 returns CSV '{returns_csv_path}' does not exist.")

    artifact_root_path = _resolve_artifact_root_path(settings)
    artifact_root_path.mkdir(parents=True, exist_ok=True)

    with app.state.main2_launch_lock:
        active_job = app.state.main2_active_launch
        process = app.state.main2_launch_process
        if active_job is not None and process is not None and process.poll() is None:
            raise HTTPException(status_code=409, detail="A main2 pipeline run is already in progress.")

        timestamp_token = datetime.now(UTC).strftime("%Y%m%d_%H%M%S")
        artifact_token = _sanitize_launch_label(artifact_label or preset["default_artifact_label"])
        artifact_id = f"main2_{artifact_token}_{timestamp_token}"
        output_prefix = artifact_root_path / artifact_id
        ...
        log_path = artifact_root_path / "launch_logs" / f"{artifact_id}.log"
        progress_log_path = artifact_root_path / "launch_logs" / f"{artifact_id}.progress.jsonl"
        _ensure_parent_directory(log_path)

        command = [
            str(_resolve_python_executable_path(settings)),
            "-m",
            "wealth_first.main2",
            "--returns-csv",
            str(returns_csv_path),
            "--benchmark-column",
            "SPY_BENCHMARK",
            "--output-prefix",
            str(output_prefix),
            "--compare-detail-csv",
            str(_resolve_main2_compare_detail_csv_path(settings)),
            "--progress-log",
        ...
        app.state.main2_launch_process = process
        app.state.main2_launch_log_handle = log_handle
        app.state.main2_active_launch = job
        return job


def _build_pipeline_launch_snapshot(settings: BridgeSettings, app: FastAPI) -> dict[str, Any]:
    _refresh_main2_launch_state(app)
    compare_detail_csv_path = _resolve_main2_compare_detail_csv_path(settings)
    return {
        "generated_at": datetime.now(UTC).isoformat(),
        "artifact_root_path": str(_resolve_artifact_root_path(settings)),
        "returns_csv_path": str(_resolve_main2_returns_csv_path(settings)),
        "compare_detail_csv_path": str(compare_detail_csv_path),
        "compare_detail_csv_exists": compare_detail_csv_path.exists(),
        "python_executable_path": str(_resolve_python_executable_path(settings)),
        "can_launch": app.state.main2_active_launch is None,
        "presets": _build_main2_launch_presets(),
        "active_job": _serialize_main2_launch_job(app.state.main2_active_launch),
        "latest_job": _serialize_main2_launch_job(app.state.main2_latest_launch),
    }
...
def _artifact_label(artifact_id: str) -> str:
    label = artifact_id
    if label.startswith("main2_"):
        label = label[len("main2_") :]
...
def _build_main2_experiment_record(summary_path: Path) -> dict[str, Any]:
    artifact_id = summary_path.name[: -len("_summary.json")]
    artifact_root = summary_path.parent
    detail_path = artifact_root / f"{artifact_id}_detail.csv"
    comparison_path = artifact_root / f"{artifact_id}_vs_current_best_summary.json"

    summary = _load_json_file(summary_path)
    comparison_summary = _load_json_file(comparison_path) if comparison_path.exists() else None
    overall = summary.get("overall") if isinstance(summary.get("overall"), dict) else {}
    weak_rows = summary.get("weak_rows") if isinstance(summary.get("weak_rows"), dict) else {}
    ...
    return {
        ...
        "comparison_path": str(comparison_path) if comparison_path.exists() else None,
        "modified_at": datetime.fromtimestamp(summary_path.stat().st_mtime, tz=UTC).isoformat(),
        "leaderboard_metrics": {
            "mean_policy_total_return": _safe_float(overall.get("mean_policy_total_return")),
            "mean_delta_total_return_vs_static_hold": _safe_float(overall.get("mean_delta_total_return_vs_static_hold")),
            "mean_delta_total_return_vs_optimizer": _safe_float(overall.get("mean_delta_total_return_vs_optimizer")),
            "policy_beats_static_hold_rows": int(overall.get("policy_beats_static_hold_rows", 0)),
            "policy_beats_optimizer_rows": int(overall.get("policy_beats_optimizer_rows", 0)),
            "rows": int(overall.get("rows", 0)),
        },
        "comparison_metrics": {
            "mean_policy_total_return_diff": _safe_float(comparison_summary.get("mean_policy_total_return_diff")) if comparison_summary else None,
            "main2_win_rows": int(comparison_summary.get("main2_win_rows", 0)) if comparison_summary else None,
            "current_win_rows": int(comparison_summary.get("current_win_rows", 0)) if comparison_summary else None,
        },
        "weak_rows_preview": _json_ready(weak_rows.get("worst_vs_static_hold", [])[:3]),
    }


def _load_main2_experiment_records(settings: BridgeSettings) -> list[dict[str, Any]]:
    artifact_root = _resolve_artifact_root_path(settings)
    if not artifact_root.exists():
        return []

    records = [
        _build_main2_experiment_record(summary_path)
        for summary_path in artifact_root.glob("main2*_summary.json")
        if not summary_path.name.endswith("_vs_current_best_summary.json")
    ]

    return sorted(
        records,
        key=lambda record: (
            record["comparison_metrics"]["mean_policy_total_return_diff"]
            if record["comparison_metrics"]["mean_policy_total_return_diff"] is not None
            else float("-inf"),
            record["leaderboard_metrics"]["mean_delta_total_return_vs_static_hold"]
            if record["leaderboard_metrics"]["mean_delta_total_return_vs_static_hold"] is not None
            else float("-inf"),
            record["modified_at"],
        ),
        reverse=True,
...
def _build_pipeline_overview(settings: BridgeSettings, app: FastAPI, limit: int) -> dict[str, Any]:
    experiments = _load_main2_experiment_records(settings)
    health = _build_health_payload(settings, app)
    event_count, _ = _tail_jsonl_records(settings.event_log_path, 1)
    execution_count, _ = _tail_jsonl_records(settings.execution_log_path, 1)
    returns_series = _read_returns_series(settings.output_csv_path, limit=1)
    recommended_experiment_id = experiments[0]["artifact_id"] if experiments else None
    latest_experiment = max(experiments, key=lambda record: record["modified_at"]) if experiments else None

    return {
        "generated_at": datetime.now(UTC).isoformat(),
        "approach": {
            "id": "adaptive-main2-distributional-rl",
            "label": "Adaptive Deep/Distributional RL",
            "module": "wealth_first.main2",
            "model_family": "Quantile Q Network",
            "description": "Adaptive SPY-versus-cash policy selection using quantile/distributional reinforcement learning with chrono and regime walk-forward evaluation.",
        },
        "artifact_root_path": str(_resolve_artifact_root_path(settings)),
        "available_experiment_count": len(experiments),
        "recommended_experiment_id": recommended_experiment_id,
        "latest_experiment_id": latest_experiment["artifact_id"] if latest_experiment else None,
        "pipeline_nodes": [
            {
                "id": "bridge",
                "label": "Webhook intake",
                "status": "live" if health["worker_running"] else "idle",
                "detail": f"{event_count} events accepted",
            },
            {
                "id": "truth",
                "label": "Normalized truth stream",
                "status": "ready" if returns_series["row_count"] > 0 else "empty",
                "detail": f"{returns_series['row_count']} return rows available",
            },
            {
                "id": "adaptive-main2",
                "label": "Adaptive DRL experiments",
                "status": "ready" if experiments else "missing",
                "detail": f"{len(experiments)} main2 experiments discovered",
            },
            {
                "id": "execution",
                "label": "Execution handoff",
                "status": "ready" if execution_count > 0 else "idle",
                "detail": f"{execution_count} execution records observed",
            },
        ],
...
def _build_pipeline_experiment_detail(settings: BridgeSettings, artifact_id: str, detail_limit: int) -> dict[str, Any]:
    experiments = _load_main2_experiment_records(settings)
    experiment = next((record for record in experiments if record["artifact_id"] == artifact_id), None)
    if experiment is None:
        raise HTTPException(status_code=404, detail=f"Unknown experiment '{artifact_id}'.")

    summary_path = Path(experiment["summary_path"])
    detail_path = Path(experiment["detail_path"]) if experiment["detail_path"] is not None else None
    comparison_path = Path(experiment["comparison_path"]) if experiment["comparison_path"] is not None else None
    ...
    return {
        ...
        "summary": _json_ready(_load_json_file(summary_path)),
        "comparison_summary": _json_ready(_load_json_file(comparison_path)) if comparison_path is not None and comparison_path.exists() else None,
        "detail": _read_detail_table(detail_path, limit=detail_limit),
    }
...
def create_app(settings: BridgeSettings) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            _cleanup_main2_launch_process(app)
    ...
    app.state.main2_launch_lock = threading.Lock()
    app.state.main2_active_launch = None
    app.state.main2_latest_launch = None
    app.state.main2_launch_process = None
    app.state.main2_launch_log_handle = None
    ...
    @app.get("/api/pipeline/experiments")
    def pipeline_experiments(limit: int = Query(default=24, ge=1, le=100)) -> dict[str, Any]:
        return _build_pipeline_overview(settings, app, limit=limit)

    @app.get("/api/pipeline/experiments/{artifact_id}")
    def pipeline_experiment_detail(
        artifact_id: str,
        detail_limit: int = Query(default=120, ge=1, le=1000),
    ) -> dict[str, Any]:
        return _build_pipeline_experiment_detail(settings, artifact_id, detail_limit=detail_limit)

    @app.get("/api/pipeline/launch")
    def pipeline_launch_status() -> dict[str, Any]:
        return _build_pipeline_launch_snapshot(settings, app)

    @app.post("/api/pipeline/launch")
    async def pipeline_launch(request: Request) -> dict[str, Any]:
        raw_body = await request.body()
        if raw_body.strip():
            try:
                payload = json.loads(raw_body.decode("utf-8"))
            except json.JSONDecodeError as exc:
                raise HTTPException(status_code=400, detail="Launch payload is not valid JSON.") from exc
            if not isinstance(payload, dict):
                raise HTTPException(status_code=400, detail="Launch payload must decode to a JSON object.")
        else:
            payload = {}
        ...
        if artifact_label is not None and not isinstance(artifact_label, str):
            raise HTTPException(status_code=400, detail="artifact_label must be a string when provided.")

        job = _start_main2_launch(settings, app, preset_id.strip(), artifact_label)
        return {
            "status": "accepted",
            "job": _serialize_main2_launch_job(job),
        }
    ...
    @app.get("/api/config/main4-promoted")
    def get_promoted_main4_config() -> dict[str, Any]:
        """Return the promoted main4 policy configuration."""
        if settings.main4_ridge_l2 is None:
            raise HTTPException(status_code=404, detail="Promoted main4 configuration not available.")
        ...
        return {
            ...
            "policy": {
                ...
                "gate": settings.main4_gate,
                "gate_scale": settings.main4_gate_scale,
                "min_robust_min_relative": settings.main4_min_robust_min_relative,
                "n_folds": settings.main4_n_folds,
                "seed": settings.main4_seed,
            },
            "promotion_metadata": {
                "date": settings.main4_promotion_date,
                "status": settings.main4_promotion_status,
                "label": settings.main4_promotion_label,
                "friction_range": settings.main4_promotion_friction_range,
            },
            "deployment_cli_command": (
                ...
                f"--gate {settings.main4_gate} "
                f"--gate-scale {settings.main4_gate_scale} "
                f"--min-robust-min-relative {settings.main4_min_robust_min_relative} "
                f"--n-folds {settings.main4_n_folds} "
                f"--seed {settings.main4_seed}"
            ),
        }
