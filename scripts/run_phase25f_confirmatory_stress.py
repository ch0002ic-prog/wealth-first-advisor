...
RUN_ROOT = ARTIFACT_ROOT / "main4_phase25f_runs"
...
# Strict target policy from latest success objective.
POLICY = {
...
def evaluate_scenario(df: pd.DataFrame) -> dict[str, Any]:
    ...
    resolved_before = mean_path_p05
    floor = POLICY["configured_min_path_bootstrap_robust_min_p05"] - POLICY["strict_path_bootstrap_gate_max_relaxation"]
    resolved_after = max(resolved_before, floor)
    cap_applied = resolved_after != resolved_before

    path_slack = mean_path_p05 - resolved_after
    activity_slack = mean_executed_step_rate - POLICY["min_mean_executed_step_rate"]
    strict_eligible = bool(pass_non_path and (mean_path_p05 >= resolved_after))
    ...
    return {
        ...
        "resolved_path_threshold_before_cap": resolved_before,
        "resolved_path_threshold_after_cap": resolved_after,
...
def main() -> int:
    ...
    for scenario in SCENARIOS:
        ...
        assessed = evaluate_scenario(sub)
    ...
    all_pass = bool(scenario_df["strict_eligible"].all())

    note = {
        "candidate_variant": WINNER.name,
        "policy": POLICY,
        "confirmatory_scenarios": SCENARIOS,
        "all_scenarios_strict_eligible": all_pass,
        "strict_eligible_scenario_count": int(scenario_df["strict_eligible"].sum()),
        "total_scenarios": int(len(SCENARIOS)),
        "min_activity_slack": float(scenario_df["activity_slack"].min()),
        "min_path_slack": float(scenario_df["path_slack"].min()),
        "max_worst_relative_drawdown": float(scenario_df["worst_relative_drawdown"].max()),
        "min_worst_robust_min": float(scenario_df["worst_robust_min"].min()),
        "decision": "PASS" if all_pass else "FAIL",
    }

    out_detail = ARTIFACT_ROOT / "main4_phase25f_confirmatory_detail.csv"
    out_summary_csv = ARTIFACT_ROOT / "main4_phase25f_confirmatory_summary.csv"
    out_summary_json = ARTIFACT_ROOT / "main4_phase25f_confirmatory_summary.json"
    out_note_json = ARTIFACT_ROOT / "main4_phase25f_promotion_note.json"

    detail.sort_values(["scenario", "transaction_cost_bps", "seed"]).to_csv(out_detail, index=False)
    scenario_df.to_csv(out_summary_csv, index=False)
    out_summary_json.write_text(json.dumps({"rows": scenario_rows}, indent=2))
