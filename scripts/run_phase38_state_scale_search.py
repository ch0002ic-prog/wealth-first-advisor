...
RUN_ROOT = ARTIFACT_ROOT / "main4_phase38_runs"
...
VARIANTS = [
    ...
    Variant(
        name="v38_state_scale_defensive",
        feature_family="baseline",
        signal_model_family="state_scaled_linear",
        regime_drawdown_threshold=-0.08,
        regime_drawdown_floor=-0.20,
        state_scale_slope_min=-1.4,
        state_scale_slope_max=0.0,
        **BASE_PARAMS,
    ),
    Variant(
        name="v38_state_scale_balanced",
        feature_family="baseline",
        signal_model_family="state_scaled_linear",
...
def run_case(variant: Variant, scenario: dict[str, Any], seed: int, cost: float, reps: int, label: str) -> dict[str, Any]:
    run_name = (
        f"phase38_{label}_{variant.name}_{scenario['name']}_r{reps}_c{int(cost)}_s{seed}"
        ...
        rows.append(
            {
                ...
                "regime_drawdown_threshold": float(regime_drawdown_threshold),
                "regime_drawdown_floor": float(regime_drawdown_floor),
                "state_scale_slope_min": float(state_scale_slope_min),
                "state_scale_slope_max": float(state_scale_slope_max),
                "rows": int(len(grp)),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase38 state-scaled mechanism search for main4")
    parser.add_argument("--label", default="state_scale_a")
    ...
    print(f"Phase38 state-scale search: label={args.label} reps={args.reps} cases={total}")
    ...
    baseline_row = rank.loc[rank["variant"] == "v38_single_linear_ref"].iloc[0].to_dict()

    detail_path = ARTIFACT_ROOT / f"main4_phase38_{args.label}_detail.csv"
    rank_path = ARTIFACT_ROOT / f"main4_phase38_{args.label}_rank.csv"
    summary_path = ARTIFACT_ROOT / f"main4_phase38_{args.label}_summary.json"
    ...
    summary = {
        "phase": "Phase38",
        "title": "State-Scaled Mechanism Search",
        "label": args.label,
        "reps": args.reps,
        "fixed_path_threshold": FIXED_PATH_THRESHOLD,
        "search_hypothesis": "State-conditional signal scaling can preserve robustness while improving returns versus a fixed-scale signal.",
        "winner": best_row,
