...
RUN_ROOT = ARTIFACT_ROOT / "main4_phase39_runs"
...
class Variant:
    ...
    state_deadband_relaxation: float
    state_deadband_min_ratio: float
    ridge_l2: float
...
VARIANTS = [
    Variant(
        ...
        state_deadband_relaxation=0.0,
        state_deadband_min_ratio=0.5,
        **BASE_PARAMS,
    ),
    Variant(
        name="v39_state_scale_base",
        feature_family="baseline",
        signal_model_family="state_scaled_linear",
        regime_drawdown_threshold=-0.10,
        regime_drawdown_floor=-0.22,
        state_scale_slope_min=-1.2,
        state_scale_slope_max=0.3,
        state_deadband_relaxation=0.0,
        state_deadband_min_ratio=0.5,
        **BASE_PARAMS,
    ),
    Variant(
        name="v39_bandaware_mild",
        feature_family="baseline",
        signal_model_family="state_scaled_linear",
        regime_drawdown_threshold=-0.10,
        regime_drawdown_floor=-0.22,
        state_scale_slope_min=-1.2,
        state_scale_slope_max=0.3,
        state_deadband_relaxation=0.35,
        state_deadband_min_ratio=0.65,
        **BASE_PARAMS,
    ),
    Variant(
        name="v39_bandaware_relaxed",
        feature_family="baseline",
        signal_model_family="state_scaled_linear",
        regime_drawdown_threshold=-0.10,
        regime_drawdown_floor=-0.22,
        state_scale_slope_min=-1.2,
        state_scale_slope_max=0.3,
        state_deadband_relaxation=0.55,
        state_deadband_min_ratio=0.50,
        **BASE_PARAMS,
    ),
]
...
def run_case(variant: Variant, scenario: dict[str, Any], seed: int, cost: float, reps: int, label: str) -> dict[str, Any]:
    run_name = (
        f"phase39_{label}_{variant.name}_{scenario['name']}_r{reps}_c{int(cost)}_s{seed}"
    ...
    cmd = [
        ...
        str(variant.state_deadband_relaxation),
        "--state-deadband-min-ratio",
        str(variant.state_deadband_min_ratio),
    ...
    if res.returncode != 0 and not detailed_path.exists():
        return {
            ...
            "state_deadband_relaxation": variant.state_deadband_relaxation,
            "state_deadband_min_ratio": variant.state_deadband_min_ratio,
...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    keys = [
        ...
        "state_deadband_relaxation",
        "state_deadband_min_ratio",
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase39 state-scale deadband-aware search for main4")
    parser.add_argument("--label", default="state_scale_bandaware_a")
    ...
    print(f"Phase39 state-scale bandaware search: label={args.label} reps={args.reps} cases={total}")
    ...
    baseline_row = rank.loc[rank["variant"] == "v39_single_linear_ref"].iloc[0].to_dict()

    detail_path = ARTIFACT_ROOT / f"main4_phase39_{args.label}_detail.csv"
    rank_path = ARTIFACT_ROOT / f"main4_phase39_{args.label}_rank.csv"
    summary_path = ARTIFACT_ROOT / f"main4_phase39_{args.label}_summary.json"
    ...
    summary = {
        "phase": "Phase39",
        "title": "State-Scale Deadband-Aware Search",
        "label": args.label,
        "reps": args.reps,
        "fixed_path_threshold": FIXED_PATH_THRESHOLD,
        "search_hypothesis": "Stress-aware deadband relaxation can reduce under-execution in state-scaled models and recover return without breaking robustness.",
        "winner": best_row,
