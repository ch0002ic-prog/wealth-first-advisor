...
RUN_ROOT = ARTIFACT_ROOT / "main4_phase37_runs"
...
VARIANTS = [
    ...
    Variant(
        name="v37_regime_two_model_dd08",
        feature_family="baseline",
    ...
    Variant(
        name="v37_regime_two_model_dd10",
        feature_family="baseline",
        signal_model_family="regime_two_model",
...
def run_case(variant: Variant, scenario: dict[str, Any], seed: int, cost: float, reps: int, label: str) -> dict[str, Any]:
    run_name = (
        f"phase37_{label}_{variant.name}_{scenario['name']}_r{reps}_c{int(cost)}_s{seed}"
...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (variant, feature_family, signal_model_family, regime_drawdown_threshold), grp in df.groupby(
        ["variant", "feature_family", "signal_model_family", "regime_drawdown_threshold"],
        dropna=False,
    ):
        rows.append(
            {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase37 regime-model mechanism search for main4")
    parser.add_argument("--label", default="regime_model_a")
    ...
    print(f"Phase37 regime-model search: label={args.label} reps={args.reps} cases={total}")
    ...
    baseline_row = rank.loc[rank["variant"] == "v37_single_linear_ref"].iloc[0].to_dict()

    detail_path = ARTIFACT_ROOT / f"main4_phase37_{args.label}_detail.csv"
    rank_path = ARTIFACT_ROOT / f"main4_phase37_{args.label}_rank.csv"
    summary_path = ARTIFACT_ROOT / f"main4_phase37_{args.label}_summary.json"
    ...
    summary = {
        "phase": "Phase37",
        "title": "Regime-Two-Model Mechanism Search",
        "label": args.label,
        "reps": args.reps,
        "fixed_path_threshold": FIXED_PATH_THRESHOLD,
        "search_hypothesis": "A state-dependent two-model signal family may improve robustness without another local threshold nudge.",
        "winner": best_row,
