...
RUN_ROOT = ARTIFACT_ROOT / "main4_phase25b_runs"
...
VARIANTS = [
    Variant(name="promoted_tanh_base"),
    Variant(
        name="promoted_tanh_tailguard_mild",
        action_smoothing=1.05,
        scale_turnover_penalty=1.0,
        no_trade_band=0.019,
        max_signal_scale=0.72,
    ),
    Variant(
        name="promoted_tanh_tailguard_med",
        action_smoothing=1.10,
        scale_turnover_penalty=2.0,
        no_trade_band=0.020,
        max_signal_scale=0.68,
    ),
    Variant(
        name="promoted_tanh_activity_boost",
        action_smoothing=0.98,
        scale_turnover_penalty=0.25,
        no_trade_band=0.016,
        max_signal_scale=0.80,
    ),
    Variant(
        name="promoted_tanh_robust_ridge",
        ridge_l2=0.020,
        action_smoothing=1.06,
        scale_turnover_penalty=1.25,
        no_trade_band=0.019,
        min_signal_scale=-0.70,
        max_signal_scale=0.70,
    ),
]
...
def summarize_variant(df: pd.DataFrame) -> pd.DataFrame:
    g = df.groupby("variant", as_index=False).agg(
...
def main() -> int:
    ...
    policy_specs = [
        {"policy_id": "strict_failmode_reference", "min_step": 0.02, "configured_path": -0.006, "quantile": 0.5, "cap": 0.001},
        {"policy_id": "strict_boundary_probe", "min_step": 0.019, "configured_path": -0.006, "quantile": 0.5, "cap": 0.001},
    ]
    ...
    out_json = ARTIFACT_ROOT / "main4_phase25b_promoted_variant_comparison.json"
    out_csv = ARTIFACT_ROOT / "main4_phase25b_promoted_variant_comparison.csv"
    out_detail = ARTIFACT_ROOT / "main4_phase25b_promoted_variant_detail.csv"

    out_detail_df = detail.sort_values(["variant", "transaction_cost_bps", "seed"])
    out_detail_df.to_csv(out_detail, index=False)
