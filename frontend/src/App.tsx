...
const ExperimentLeaderboardChart = lazy(() => import("./charts/ExperimentLeaderboardChart"));
...
type ExperimentSortKey = "comparison" | "static" | "optimizer" | "return" | "wins";
type ExperimentFilterMode = "all" | "positive_current" | "majority_static" | "majority_optimizer";
...
const EXPERIMENT_SORT_KEYS: ExperimentSortKey[] = ["comparison", "static", "optimizer", "return", "wins"];
const EXPERIMENT_FILTER_MODES: ExperimentFilterMode[] = ["all", "positive_current", "majority_static", "majority_optimizer"];
...
function usePollingJson<T>(url: string | null, intervalMs: number): PollingState<T> {
...
function formatSignedPercentage(value: number | null | undefined, digits = 2): string {
  if (value == null || !Number.isFinite(value)) {
    return "-";
  }
...
function isExperimentSortKey(value: string | null): value is ExperimentSortKey {
  return value !== null && EXPERIMENT_SORT_KEYS.includes(value as ExperimentSortKey);
}

function isExperimentFilterMode(value: string | null): value is ExperimentFilterMode {
  return value !== null && EXPERIMENT_FILTER_MODES.includes(value as ExperimentFilterMode);
}
...
function readExperimentViewUrlState(): {
  ...
  return {
    ...
    experimentSort: isExperimentSortKey(sortParam) ? sortParam : DEFAULT_EXPERIMENT_SORT,
    experimentFilter: isExperimentFilterMode(filterParam) ? filterParam : DEFAULT_EXPERIMENT_FILTER,
  };
}

function syncQueryParam(params: URLSearchParams, key: string, value: string | null | undefined, defaultValue?: string): void {
...
function sortWeakRows(rows: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  return [...rows].sort(
    (left, right) => numberValue(left.delta_total_return_vs_static_hold) - numberValue(right.delta_total_return_vs_static_hold)
  );
}

function differenceOrNull(left: number | null | undefined, right: number | null | undefined): number | null {
...
function experimentSortMetric(experiment: ExperimentRecord, sortKey: ExperimentSortKey): number {
  switch (sortKey) {
    case "return":
      return experiment.leaderboard_metrics.mean_policy_total_return ?? Number.NEGATIVE_INFINITY;
    case "static":
      return experiment.leaderboard_metrics.mean_delta_total_return_vs_static_hold ?? Number.NEGATIVE_INFINITY;
    case "optimizer":
      return experiment.leaderboard_metrics.mean_delta_total_return_vs_optimizer ?? Number.NEGATIVE_INFINITY;
    case "wins":
      return experiment.leaderboard_metrics.rows > 0
        ? experiment.leaderboard_metrics.policy_beats_static_hold_rows / experiment.leaderboard_metrics.rows
...
function App() {
  ...
  const pipelineOverview = usePollingJson<PipelineOverview>("/api/pipeline/experiments?limit=64", PIPELINE_POLL_MS);
  const pipelineLaunch = usePollingJson<PipelineLaunchState>("/api/pipeline/launch", PIPELINE_LAUNCH_POLL_MS);

  const [initialExperimentView] = useState(readExperimentViewUrlState);

  const [selectedExperimentId, setSelectedExperimentId] = useState<string | null>(initialExperimentView.selectedExperimentId);
  const [comparisonExperimentId, setComparisonExperimentId] = useState<string | null>(initialExperimentView.comparisonExperimentId);
  const [experimentQuery, setExperimentQuery] = useState(initialExperimentView.experimentQuery);
  const [experimentSort, setExperimentSort] = useState<ExperimentSortKey>(initialExperimentView.experimentSort);
  const [experimentFilter, setExperimentFilter] = useState<ExperimentFilterMode>(initialExperimentView.experimentFilter);
  ...
  const [processedLaunchSignature, setProcessedLaunchSignature] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const selectionIsValid =
      selectedExperimentId !== null && experiments.some((experiment) => experiment.artifact_id === selectedExperimentId);
    if (!selectionIsValid) {
      setSelectedExperimentId(pipelineOverview.data?.recommended_experiment_id ?? experiments[0].artifact_id);
    }
  ...
  useEffect(() => {
    ...
    const comparisonIsValid =
      ...
      experiments.some((experiment) => experiment.artifact_id === comparisonExperimentId);
    ...
    const fallbackComparison =
      experiments.find(
        (experiment) =>
          ...
          experiment.artifact_id === pipelineOverview.data?.recommended_experiment_id
      ) ?? experiments.find((experiment) => experiment.artifact_id !== selectedExperimentId);
  ...
  useEffect(() => {
    ...
    syncQueryParam(params, "filter", experimentFilter, DEFAULT_EXPERIMENT_FILTER);
  ...
  }, [comparisonExperimentId, experimentFilter, experimentQuery, experimentSort, selectedExperimentId]);
  ...
  useEffect(() => {
    ...
    const selectedPresetIsValid = presets.some((preset) => preset.id === selectedLaunchPresetId);
    if (!selectedPresetIsValid) {
      setSelectedLaunchPresetId(presets.find((preset) => preset.recommended)?.id ?? presets[0].id);
    }
  }, [pipelineLaunch.data, selectedLaunchPresetId]);

  const experimentDetail = usePollingJson<ExperimentDetail>(
    selectedExperimentId ? `/api/pipeline/experiments/${selectedExperimentId}?detail_limit=120` : null,
    DETAIL_POLL_MS
  );
  const comparisonDetail = usePollingJson<ExperimentDetail>(
    comparisonExperimentId ? `/api/pipeline/experiments/${comparisonExperimentId}?detail_limit=120` : null,
    DETAIL_POLL_MS
  );
  ...
  const handleLaunchMain2 = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({ detail: "Launcher did not return JSON." }));
      if (!response.ok) {
        throw new Error(typeof body.detail === "string" ? body.detail : `Launch failed with status ${response.status}`);
      }

      setLaunchSubmitStatus(`Started ${String(body.job?.artifact_id ?? selectedLaunchPresetId)}.`);
      setLaunchArtifactLabel("");
      pipelineLaunch.refresh();
    } catch (caughtError) {
      setLaunchSubmitStatus(caughtError instanceof Error ? caughtError.message : "Launch request failed.");
    } finally {
  ...
  }, [pipelineLaunch.data?.latest_job, pipelineOverview, processedLaunchSignature]);
  ...
  const filteredExperiments = [...allExperiments]
    .filter((experiment) => compactLabel(experiment.label).toLowerCase().includes(queryText))
    .filter((experiment) => {
      if (experimentFilter === "positive_current") {
        return (experiment.comparison_metrics.mean_policy_total_return_diff ?? Number.NEGATIVE_INFINITY) > 0;
      }
    ...
    .sort((left, right) => experimentSortMetric(right, experimentSort) - experimentSortMetric(left, experimentSort));
  ...
  const leaderboardRows = filteredExperiments.map((experiment) => ({
    ...
    comparisonScore:
      ...
      experiment.leaderboard_metrics.mean_delta_total_return_vs_static_hold ??
      0,
    policyTotalReturn: experiment.leaderboard_metrics.mean_policy_total_return ?? 0,
  ...
  const hardestWindows = sortWeakRows(detailRows).slice(0, 10).map((row) => ({
  ...
  const weakRows =
    (selectedDetail?.summary.weak_rows as { worst_vs_static_hold?: Array<Record<string, unknown>> } | undefined)?.worst_vs_static_hold ?? [];

  const comparisonRowMap = new Map(comparisonRows.map((row) => [buildWindowKey(row), row]));
  const sharedWindowComparisons = detailRows
    .map((row) => {
      ...
      const comparisonEdge = numberValue(comparisonRow.delta_total_return_vs_static_hold);
      return {
    ...
    .sort((left, right) => Math.abs(right.edgeDelta) - Math.abs(left.edgeDelta));

  const selectedComparisonWins = sharedWindowComparisons.filter((row) => row.edgeDelta > 0).length;
  const comparisonComparisonWins = sharedWindowComparisons.filter((row) => row.edgeDelta < 0).length;
  const meanComparisonEdge =
    sharedWindowComparisons.length > 0
      ? sharedWindowComparisons.reduce((total, row) => total + row.edgeDelta, 0) / sharedWindowComparisons.length
      : null;
      ...
      ? [
          {
            ...
            value: differenceOrNull(
              selectedDetail.experiment.leaderboard_metrics.mean_policy_total_return,
              comparisonExperimentDetail.experiment.leaderboard_metrics.mean_policy_total_return
            ),
          },
          {
            label: "Vs static gap",
            value: differenceOrNull(
              selectedDetail.experiment.leaderboard_metrics.mean_delta_total_return_vs_static_hold,
              comparisonExperimentDetail.experiment.leaderboard_metrics.mean_delta_total_return_vs_static_hold
            ),
          },
          {
            label: "Vs optimizer gap",
            value: differenceOrNull(
              selectedDetail.experiment.leaderboard_metrics.mean_delta_total_return_vs_optimizer,
              comparisonExperimentDetail.experiment.leaderboard_metrics.mean_delta_total_return_vs_optimizer
            ),
          },
          {
            label: "Vs current-best gap",
            value: differenceOrNull(
              selectedDetail.experiment.comparison_metrics.mean_policy_total_return_diff,
              comparisonExperimentDetail.experiment.comparison_metrics.mean_policy_total_return_diff
            ),
          },
        ]
      : [];
  const selectedLaunchPreset = launchState?.presets.find((preset) => preset.id === selectedLaunchPresetId) ?? null;
  const currentLaunchJob = launchState?.active_job ?? launchState?.latest_job ?? null;
  ...
  const overallLaunchProgress = clampUnitInterval(launchProgress?.overall_progress_fraction);
  const currentFoldProgress = clampUnitInterval(launchProgress?.step_progress_fraction);
  const launchProgressContext = [
    launchProgress?.active_split,
    launchProgress?.active_fold,
    launchProgress?.active_seed != null ? `seed ${formatCount(launchProgress.active_seed)}` : null,
  ]
    .filter((value): value is string => Boolean(value))
    .join(" · ");
  const launchLogText = currentLaunchJob?.log_tail.length ? currentLaunchJob.log_tail.join("\n") : "No launcher logs yet.";
  ...
  return (
    <div className="app-shell">
      ...
      <header className="hero">
        <div className="hero__copy">
          ...
          <p className="hero__lede">
            Real-time webhook intake, normalized truth monitoring, and experiment tracking for the adaptive Deep/Distributional
            RL pipeline.
          </p>
        </div>
      ...
      <main className="page-grid">
        <section className="panel-card panel-card--wide">
          <div className="section-heading">
            ...
            <p className="muted">
              {pipeline?.approach.description ?? "Loading adaptive pipeline metadata..."}
            </p>
          </div>
          <div className="pipeline-rail">
            {(pipeline?.pipeline_nodes ?? []).map((node) => (
              <article key={node.id} className="pipeline-node">
                <span className={`pipeline-node__state pipeline-node__state--${node.status}`}>{node.status}</span>
                <h3>{node.label}</h3>
                <p>{node.detail}</p>
              </article>
            ))}
        ...
        <section className="panel-card panel-card--wide">
          <div className="section-heading">
            ...
            <p className="muted">Launch real <code>wealth_first.main2</code> runs from localhost and watch the board pick up the resulting artifacts.</p>
          </div>

          <form className="launcher-form" onSubmit={handleLaunchMain2}>
            <label>
              <span>Launch preset</span>
              <select value={selectedLaunchPresetId} onChange={(event) => setSelectedLaunchPresetId(event.target.value)}>
            ...
            <label>
              ...
              <input
                ...
                placeholder={selectedLaunchPreset?.default_artifact_label ?? "live_quick_probe"}
              />
            </label>
            ...
            <div className="launcher-form__actions">
              <button
                ...
                type="submit"
                disabled={launchSubmitting || launchState?.can_launch === false || !selectedLaunchPresetId}
              >
          ...
          {selectedLaunchPreset && (
            <p className="muted launcher-description">
              {selectedLaunchPreset.description} Estimated runtime: {selectedLaunchPreset.estimated_runtime}.
            </p>
          )}
            ...
            <div className="comparison-shell comparison-shell--launch">
              <div className="comparison-shell__header">
                <div>
                  ...
                  <p className="muted">
                    {currentLaunchJob.artifact_id} · started {formatDateTime(currentLaunchJob.started_at)}
                    {currentLaunchJob.completed_at ? ` · finished ${formatDateTime(currentLaunchJob.completed_at)}` : ""}
                  </p>
                </div>
                <span className={`pipeline-node__state ${launchStatusClass(currentLaunchJob.status)}`}>{currentLaunchJob.status}</span>
              </div>
              ...
              <div className="comparison-grid">
                ...
                <article className="metric-block metric-block--coral metric-block--delta">
                  ...
                  <strong>
                    {formatCount(
                      [currentLaunchJob.summary_exists, currentLaunchJob.detail_exists, currentLaunchJob.comparison_summary_exists].filter(Boolean)
                        .length
                    )}
                  </strong>
                </article>
              ...
              <div className="launch-progress-shell">
                <div className="launch-progress__header">
                  <div>
                    ...
                    <h4>{launchProgress?.stage_label ?? "Waiting for telemetry"}</h4>
                  </div>
                  <p className="muted">
                    {launchProgressContext || "Structured fold and checkpoint events appear here while wealth_first.main2 trains."}
                  </p>
                </div>
                ...
                <div className="comparison-grid">
                  <article className="metric-block metric-block--teal metric-block--delta">
                    ...
                    <strong>
                      {formatCount(launchProgress?.completed_folds)} / {formatCount(launchProgress?.total_folds)}
                    </strong>
                  </article>
                  <article className="metric-block metric-block--ink metric-block--delta">
                    <span>Training step</span>
                    <strong>
                      {formatCount(launchProgress?.active_step)} / {formatCount(launchProgress?.train_steps)}
                    </strong>
                  </article>
                  <article className="metric-block metric-block--amber metric-block--delta">
                    <span>Validation score</span>
                    <strong>{formatPercentage(launchProgress?.latest_validation_score)}</strong>
                  </article>
                  <article className="metric-block metric-block--coral metric-block--delta">
                    <span>Best checkpoint</span>
                    <strong>{formatPercentage(launchProgress?.best_validation_score)}</strong>
                  </article>
                </div>
                ...
                <div className="comparison-grid launch-progress-grid--internals">
                  <article className="metric-block metric-block--ink metric-block--delta">
                    ...
                    <strong>{formatPercentage(launchProgress?.latest_epsilon, 1)}</strong>
                  </article>
                  <article className="metric-block metric-block--teal metric-block--delta">
                    <span>Replay buffer</span>
                    <strong>{formatCount(launchProgress?.latest_replay_size)}</strong>
                  </article>
                  <article className="metric-block metric-block--amber metric-block--delta">
                    <span>Episode resets</span>
                    <strong>{formatCount(launchProgress?.latest_episode_reset_count)}</strong>
                  </article>
                  <article className="metric-block metric-block--coral metric-block--delta">
                    <span>Latest loss</span>
                    <strong>{formatDecimal(launchProgress?.latest_loss, 4)}</strong>
                  </article>
                </div>
                ...
                <div className="launch-progress-bars">
                  <div className="launch-progress-bar">
                    <div className="launch-progress-bar__copy">
                      ...
                      <strong>{formatPercentage(overallLaunchProgress, 1)}</strong>
                    </div>
                    <div className="launch-progress-bar__track" aria-hidden="true">
                      <span className="launch-progress-bar__fill" style={{ width: `${overallLaunchProgress * 100}%` }} />
                    </div>
                  </div>
                  <div className="launch-progress-bar">
                    <div className="launch-progress-bar__copy">
                      <span>Current fold progress</span>
                      <strong>{formatPercentage(currentFoldProgress, 1)}</strong>
                    </div>
                    <div className="launch-progress-bar__track" aria-hidden="true">
                      <span className="launch-progress-bar__fill launch-progress-bar__fill--secondary" style={{ width: `${currentFoldProgress * 100}%` }} />
                    </div>
                  </div>
                </div>
                ...
                <div className="launch-progress-chart-shell">
                  ...
                  <div className="chart-shell chart-shell--launch-progress">
                    {launchCheckpointChartRows.length > 0 ? (
                      <Suspense fallback={<ChartFallback message="Loading checkpoint chart..." />}>
                        <LaunchProgressChart rows={launchCheckpointChartRows} />
                      </Suspense>
                    ) : (
                      <div className="empty-state empty-state--compact">Waiting for checkpoint history...</div>
                    )}
                  </div>
                </div>
                ...
                {launchProgress?.latest_fold_metrics && (
                  <div className="comparison-grid">
                    <article className="metric-block metric-block--ink metric-block--delta">
                      ...
                      <strong>{formatPercentage(launchProgress.latest_fold_metrics.test_total_return)}</strong>
                    </article>
                    <article className="metric-block metric-block--teal metric-block--delta">
                      <span>Latest fold validation return</span>
                      <strong>{formatPercentage(launchProgress.latest_fold_metrics.validation_total_return)}</strong>
                    </article>
                    <article className="metric-block metric-block--amber metric-block--delta">
                      <span>Avg SPY weight</span>
                      <strong>{formatPercentage(launchProgress.latest_fold_metrics.test_average_spy_weight)}</strong>
                    </article>
                    <article className="metric-block metric-block--coral metric-block--delta">
                      <span>Avg turnover</span>
                      <strong>{formatPercentage(launchProgress.latest_fold_metrics.test_average_turnover)}</strong>
                    </article>
                  </div>
                )}
                ...
                <div className="launch-progress-events">
                  {recentLaunchEvents.length > 0 ? (
                    recentLaunchEvents.map((event, index) => (
                      <article key={`${String(event.timestamp ?? "event")}-${String(event.event_type ?? index)}`} className="launch-progress-event">
                        <div className="launch-progress-event__header">
                          <span className="badge badge--ink">{startCaseLabel(event.event_type)}</span>
                          <span className="muted">{formatDateTime(typeof event.timestamp === "string" ? event.timestamp : null)}</span>
                        </div>
                        <strong>{formatLaunchEventContext(event)}</strong>
                        <p className="muted">{formatLaunchEventDetail(event)}</p>
                      </article>
                    ))
                  ) : (
                    <div className="empty-state empty-state--compact">Waiting for first structured DRL progress event...</div>
                  )}
                </div>
              </div>
              ...
              <pre className="response-shell response-shell--short">{launchLogText}</pre>
            </div>
          ) : (
            <div className="empty-state empty-state--compact">
              {pipelineLaunch.loading ? "Loading launcher state..." : "No main2 launch has been started from the board yet."}
            </div>
          )}
        </section>
        ...
        <section className="panel-card panel-card--wide">
          <div className="section-heading">
            ...
            <p className="muted">Fast read on ingestion, normalization, execution, and failures.</p>
          </div>
        ...
        <section className="panel-card panel-card--chart">
          ...
          <div className="chart-shell">
            {displayedAssets.length > 0 ? (
              <Suspense fallback={<ChartFallback message="Loading chart module..." />}>
                <ReturnsSeriesChart
        ...
        <section className="panel-card panel-card--stream">
          <div className="section-heading">
            ...
            <p className="muted">Filter, rank, and compare main2 distributional-RL experiments against the current best and each other.</p>
          </div>
          <div className="toolbar-grid">
            ...
            <label>
              <span>Filter</span>
              <select value={experimentFilter} onChange={(event) => setExperimentFilter(event.target.value as ExperimentFilterMode)}>
                <option value="all">All experiments</option>
                <option value="positive_current">Positive vs current best</option>
                <option value="majority_static">Majority beat static hold</option>
                <option value="majority_optimizer">Majority beat optimizer</option>
              </select>
            </label>
            <label>
              <span>Sort by</span>
              <select value={experimentSort} onChange={(event) => setExperimentSort(event.target.value as ExperimentSortKey)}>
          ...
          <p className="muted toolbar-caption">
            Showing {formatCount(filteredExperiments.length)} of {formatCount(allExperiments.length)} discovered experiments.
          </p>
          <div className="chart-shell chart-shell--short">
            {leaderboardRows.length > 0 ? (
              <Suspense fallback={<ChartFallback message="Loading chart module..." />}>
                <ExperimentLeaderboardChart
                  recommendedExperimentId={pipeline?.recommended_experiment_id ?? null}
                  rows={leaderboardRows.slice(0, 10)}
                />
              </Suspense>
            ) : (
              <div className="empty-state">{pipelineOverview.loading ? "Loading experiment board..." : "No adaptive experiment artifacts were found."}</div>
            )}
          </div>
          ...
          <div className="selector-grid">
            <div className="selector-row">
              ...
              >
                {allExperiments.map((experiment) => (
                  <option key={experiment.artifact_id} value={experiment.artifact_id}>
            ...
            <div className="selector-row">
              <label htmlFor="comparisonExperimentSelect">Compare against</label>
              <select
                id="comparisonExperimentSelect"
                value={comparisonExperimentId ?? ""}
                onChange={(event) => setComparisonExperimentId(event.target.value || null)}
              >
                {allExperiments
                  .filter((experiment) => experiment.artifact_id !== selectedExperimentId)
                  .map((experiment) => (
                    <option key={experiment.artifact_id} value={experiment.artifact_id}>
                      {compactLabel(experiment.label)}
                    </option>
          ...
          {selectedDetail ? (
            <>
              <div className="metric-grid metric-grid--compact">
                <article className="metric-block metric-block--teal">
                  ...
                  <strong>{formatPercentage(selectedDetail.experiment.leaderboard_metrics.mean_policy_total_return)}</strong>
                </article>
                <article className="metric-block metric-block--amber">
                  <span>Vs static hold</span>
                  <strong>{formatPercentage(selectedDetail.experiment.leaderboard_metrics.mean_delta_total_return_vs_static_hold)}</strong>
                </article>
                <article className="metric-block metric-block--ink">
                  <span>Vs optimizer</span>
                  <strong>{formatPercentage(selectedDetail.experiment.leaderboard_metrics.mean_delta_total_return_vs_optimizer)}</strong>
                </article>
                <article className="metric-block metric-block--coral">
                  <span>Vs current best</span>
                  <strong>{formatPercentage(selectedDetail.experiment.comparison_metrics.mean_policy_total_return_diff)}</strong>
                </article>
              </div>
              ...
              <div className="comparison-shell">
                <div className="comparison-shell__header">
                  <div>
                    ...
                    <p className="muted">
                      Stack the selected main2 run against a second artifact to see which windows actually move and by how much.
                    </p>
                  </div>
                  <div className="comparison-shell__labels">
                    <span className="badge badge--teal">selected {compactLabel(selectedDetail.experiment.label)}</span>
                    <span className="badge badge--amber">
                      compare {compactLabel(comparisonExperimentDetail?.experiment.label ?? "none")}
                    </span>
                  </div>
                </div>
                ...
                {comparisonExperimentDetail ? (
                  <>
                    <div className="metric-grid metric-grid--compact">
                      {comparisonMetricCards.map((metric) => (
                        <article key={metric.label} className="metric-block metric-block--ink metric-block--delta">
                          <span>{metric.label}</span>
                          <strong className={deltaToneClass(metric.value)}>{formatSignedPercentage(metric.value)}</strong>
                        </article>
                      ))}
                    </div>
                    ...
                    <div className="comparison-grid">
                      <article className="metric-block metric-block--ink metric-block--delta">
                        ...
                        <strong>{formatCount(sharedWindowComparisons.length)}</strong>
                      </article>
                      <article className="metric-block metric-block--teal metric-block--delta">
                      ...
                      <article className="metric-block metric-block--coral metric-block--delta">
                        ...
                        <strong className={deltaToneClass(meanComparisonEdge)}>{formatSignedPercentage(meanComparisonEdge)}</strong>
                      </article>
                    </div>
                        ...
                        <tbody>
                          {sharedWindowComparisons.slice(0, 8).map((row) => (
                            <tr key={row.key}>
                              ...
                              <td>
                                <span className={`trend-value ${deltaToneClass(row.edgeDelta)}`}>{formatSignedPercentage(row.edgeDelta)}</span>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                  ...
                  <div className="empty-state">{comparisonDetail.loading ? "Loading comparison experiment..." : "Choose a second experiment to compare."}</div>
                )}
              </div>
              ...
              <div className="weak-grid">
                <div>
                  ...
                  <p className="muted">
                    {selectedDetail.experiment.label} · updated {formatDateTime(selectedDetail.experiment.modified_at)}
                  </p>
                  <p className="muted">
                    Quantile/distributional reinforcement learning in <code>{selectedDetail.approach.module}</code> with artifact-backed evaluation.
                  </p>
                  <div className="artifact-grid">
                    <div>
                      <span className="path-grid__label">Summary</span>
                      <code>{selectedDetail.experiment.summary_path ?? "-"}</code>
                    </div>
                    <div>
                      <span className="path-grid__label">Detail CSV</span>
                      <code>{selectedDetail.experiment.detail_path ?? "-"}</code>
                    </div>
                  </div>
                </div>
                <div className="weak-table-shell">
                  <table>
                    ...
                    <tbody>
                      {sortWeakRows(weakRows).slice(0, 8).map((row, index) => (
                        <tr key={`${row.split}-${row.seed}-${row.fold}-${row.phase}-${index}`}>
                          <td>{`${String(row.split ?? "-")}/${String(row.seed ?? "-")}/${String(row.fold ?? "-")}/${String(row.phase ?? "-")}`}</td>
                          <td>{formatPercentage(numberValue(row.delta_total_return_vs_static_hold))}</td>
                          <td>{formatPercentage(numberValue(row.policy_total_return))}</td>
                          <td>{formatPercentage(numberValue(row.static_hold_total_return))}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="empty-state">{experimentDetail.loading ? "Loading selected experiment..." : "Choose an experiment to inspect."}</div>
          )}
        </section>
        ...
        <section className="panel-card panel-card--stream">
          ...
          <div className="stream-list">
            {(snapshot?.recent_events ?? []).map((event, index) => (
              <article key={`${String(event.fingerprint ?? index)}`} className="stream-card">
                <div className="stream-card__header">
                  <div>
                    <h3>{String(event.sleeve ?? "unknown sleeve")}</h3>
                    <p className="muted">{formatDateTime(String(event.timestamp ?? ""))}</p>
                  </div>
                  <span className="badge">{String(event.event_type ?? "event")}</span>
                </div>
                <div className="badge-row">
                  <span className="badge badge--teal">symbol {String(event.symbol ?? "-")}</span>
                  <span className="badge badge--amber">return {formatPercentage(numberValue(event.return_value), 2)}</span>
                  <span className="badge">{shortId(String(event.fingerprint ?? ""))}</span>
                </div>
              </article>
            ))}
          </div>
      ...
      {(dashboard.error || pipelineOverview.error || pipelineLaunch.error || experimentDetail.error || comparisonDetail.error) && (
